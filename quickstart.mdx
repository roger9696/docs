...
Log in to your [PayX Dashboard](https://payx.company/dashboard) and navigate to the **Developer Settings** tab. Copy your `Test Secret Key` (`px_test_v2_...`).
...
Create a new file and initialize the PayX client with your v2 API key.

```javascript
const { PayX } = require('payx-node');

const payx = new PayX({
  apiKey: process.env.PAYX_SECRET_KEY // or 'px_test_v2_YOUR_TEST_KEY'
});
```
...
<CardGroup cols={2}>
  ...
  <Card title="PayX JS Checkout" icon="credit-card" href="/payx-js">
    Integrate our secure inline modal checkout.
  </Card>
</CardGroup>
